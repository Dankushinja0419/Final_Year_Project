...
public class TesseractWrapper
{
    ...
    private const float MinimumConfidence = 1; // Recognization threshold
    ...
    [DllImport(TesseractDllName)]
    private static extern void TessBaseAPISetVariable(IntPtr handle, string name, string value); // Set characters to focus on
    ...
    public string Recognize(Texture2D texture)
    {
        ...
        // Filter out only Hiragana characters from the recognized text
        string hiraganaText = FilterHiragana(recognizedText);
    ...
    public void SetWhitelist(string whitelist)
    {
        ...
        {
            TessBaseAPISetVariable(_tessHandle, "tessedit_char_whitelist", whitelist); // focus on characters to be recognize
        }
    }
    ...
    public void SetBlacklist(string blacklist)
    {
        if (_tessHandle != IntPtr.Zero)
        {
            TessBaseAPISetVariable(_tessHandle, "tessedit_char_blacklist", blacklist);
        }
    }

    private string FilterHiragana(string input)
    {
        StringBuilder hiraganaOnly = new StringBuilder();

        foreach (char c in input)
        {
            // Check if character is in the Hiragana Unicode range (0x3040 to 0x309F)
            if (c >= 0x3042 && c <= 0x3093 && c != 'ゐ' && c != 'ゑ')//(c >= 0x3040 && c <= 0x309F) Full hiragana with dakuten ゛ && handakuten ゜
            {
                //hiraganaOnly.Append(c);
                return c.ToString();
            }
        }

        //return hiraganaOnly.ToString();
        return "?";
    }
